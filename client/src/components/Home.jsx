...
const Home = () => {
  ...
  const uploadCode = async () => {
    try {
      ...
      var config = {
        method: "post",
        url: "https://codex-gxvj.onrender.com",
        headers: {
          "Content-Type": "application/json",
        },
        data: data,
  ...
  const uploadCodeAndGenerateUrl = async () => {
    try {
      ...
      const res = await axios.post(
        "/api/uploadcode",
        { code: code, extension: ext, language: lang },
        newConfig
      );
    ...
    <>
      <div style={{ overflowY: "hidden", overflowX: "hidden", width: "100vw" }}>
        <div className="row" style={{ maxHeight: "100vh" }}>
          ...
          >
            <img
              src="https://see.fontimg.com/api/renderfont4/BW633/eyJyIjoiZnMiLCJoIjo5OCwidyI6MTUwMCwiZnMiOjY1LCJmZ2MiOiIjRURFMTBGIiwiYmdjIjoiI0E3MzA5QSIsInQiOjF9/Q09ESVRPUiBTVFVESU8/fase-bulan.png"
              className="img-fluid"
          ...
          >
            ...
            <button
              ...
              data-bs-toggle="tooltip"
              data-bs-placement="top"
              title="Run and generate URL"
              onClick={uploadCodeAndGenerateUrl}
            >
              <VscDebugRerun />
            </button>
          </div>
      ...
      >
        <DialogTitle id="alert-dialog-title">{linkBody}</DialogTitle>
        <DialogContent>
